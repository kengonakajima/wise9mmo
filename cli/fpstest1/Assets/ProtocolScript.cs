...
public class ProtocolScript : MonoBehaviour {
    ...
    void Start () {          // 主人公が登場したときに1回だけ呼ばれる
        ...
        // 以下の2行をstandaloneで呼び出すと通信できなくなるので注意
        Boolean pol = Security.PrefetchSocketPolicy ( "127.0.0.1", 7000, 3000);       // ポリシーファイルをサーバから　取ってくる
        Debug.Log( "policy:" + pol);
    ...
    public ArrayList readJSON() {
        ...
        while(true){
            String s = readSocket();
            if( s != "" ){
                //            Debug.Log( "sockdata:" + s );
                try {
                    outlist.Add( new JSONObject(s) );
                } catch( Exception e ) {
